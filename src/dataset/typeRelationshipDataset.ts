...
/**
 * Utility to generate inverse relationships for each entry in the dataset.
 * The inverse relation name is generated by a simple heuristic (e.g., hasX -> isXOf),
 * and the confidence is reduced (default: 0.8x original).
 */
function generateInverseRelations(
  relationships: TypeRelationship[],
  confidenceFactor = 0.8
): TypeRelationship[] {
  const inverseName = (name: string) => {
    if (name.startsWith("has")) {
      return `is${name.slice(3)}Of`;
    }
    if (name.startsWith("is") && name.endsWith("Of")) {
      return `has${name.slice(2, -2)}`;
    }
    return `${name}Inverse`;
  };
  return relationships.map(rel => ({
    ...
    relationName: inverseName(rel.relationName),
    confidence: Math.round(rel.confidence * confidenceFactor * 100) / 100,
  }));
}

// Export a version of the dataset with both direct and inverse relations
export const ALL_TYPE_RELATIONSHIPS: TypeRelationship[] = [
  ...KNOWN_TYPE_RELATIONSHIPS,
  ...generateInverseRelations(KNOWN_TYPE_RELATIONSHIPS)
];

/**
 * Index the relationships for fast lookup by sourceType and targetType
 */
export function indexRelationships(
  relationships: TypeRelationship[]
): Map<string, Map<string, TypeRelationship[]>> {
  const index = new Map<string, Map<string, TypeRelationship[]>>();
  for (const rel of relationships) {
    if (!index.has(rel.sourceType)) {
      index.set(rel.sourceType, new Map());
    }
    const targetMap = index.get(rel.sourceType);
    if (targetMap) {
      if (!targetMap.has(rel.targetType)) {
        targetMap.set(rel.targetType, []);
      }
      const relArr = targetMap.get(rel.targetType);
      if (relArr) {
        relArr.push(rel);
      }
    }
