...
export async function processSingleFileCEA(
    ...
    try {
        logger.box("Mode simple - Annotation d'entités (CEA) d'un seul fichier CSV");
        logger.info(`Fichier à traiter: ${csvFilePath}`);
        ...
        for (const res of results) {
            logger.info(`Ligne ${res.row}, Colonne ${res.column} : ${res.uri}, Confiance: ${(res.confidence * 100).toFixed(0)}%`);
        }
        logger.success("Traitement terminé avec succès !");
    } catch (error) {
        logger.error(
...
export async function processInputCsvCEA(
    ...
    try {
        ...
        const inputData: { id: string; row: number; column: number; uri?: string }[] = lines.map(line => {
        ...
        logger.info(`${inputData.length} lignes trouvées dans le fichier d'entrée`);
        if (!existsSync(csvDirectoryPath)) {
            throw new Error(`Le dossier CSV spécifié n'existe pas : ${csvDirectoryPath}`);
        }
        const files = readdirSync(csvDirectoryPath)
            .filter((file) => extname(file).toLowerCase() === ".csv")
            .map((file) => join(csvDirectoryPath, file));
        if (files.length === 0) {
            logger.warn(`Aucun fichier CSV trouvé dans le dossier ${csvDirectoryPath}`);
            return;
        }
        logger.info(`${files.length} fichiers CSV trouvés dans le dossier ${csvDirectoryPath}`);
        let processedCount = 0;
        const totalFiles = files.length;
        logger.info(`Progression globale du traitement (${totalFiles} fichiers dans le dossier) :`);
        updateProgressBar(processedCount, totalFiles);
        const processedFiles = new Set<string>();
        ...
        for (const id of ids) {
            ...
            try {
                ...
                for (const entry of inputData) {
                    ...
                    const cell = results.find((r) => r.row === entry.row - 1 && r.column === entry.column);
                    if (cell) {
                        entry.uri = cell.uri;
                        logger.info(`Résultat pour ${entry.id}, ligne ${entry.row}, colonne ${entry.column} : ${entry.uri}`);
                    } else {
                        entry.uri = "";
                        logger.warn(`Aucune entité trouvée pour la cellule (${entry.row}, ${entry.column}) dans le fichier ${csvFile}`);
                    }
                }
                ...
                logger.error(`Erreur lors du traitement du fichier ${csvFile} : ${error instanceof Error ? error.message : String(error)}`);
            }
            processedCount = processedFiles.size;
            updateProgressBar(processedCount, totalFiles);
        }
        logger.success(`Tous les résultats ont été enregistrés dans ${inputCsvPath}`);
    } catch (error) {
        logger.error(`Erreur lors du traitement du fichier d'entrée (CEA) : ${error instanceof Error ? error.message : String(error)}`);
        throw error;
    }
}
...
export async function main() {
    try {
        ...
        const annotationType = await logger.prompt("Choisissez le type d'annotation :", {
            type: "select",
            options: [
        ...
        const mode = await logger.prompt("Choisissez un mode de traitement:", {
            ...
            options: [
                ...
                {value: "batch", label: "Mode batch (traiter un lot de fichiers CSV)"},
            ],
        });
        ...
        if (mode === "simple") {
            ...
            if (!existsSync(inputCsvPath)) {
                logger.error(`Le fichier CSV d'entrée spécifié n'existe pas : ${inputCsvPath}`);
                return;
            }
            if (!existsSync(csvDirectoryPath)) {
                logger.error(`Le dossier CSV spécifié n'existe pas : ${csvDirectoryPath}`);
                return;
            }
            if (annotationType === "cta") {
        ...
        logger.error(`Erreur : ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
}
