...
export class CacheService {
  ...
  setInWikidataCache(key: string, value: any): void {
    ...
    logger.debug(`Cached (Wikidata): ${key.substring(0, 50)}${key.length > 50 ? '...' : ''}`);
  }
  ...
  setInDBpediaCache(key: string, value: any): void {
    ...
    logger.debug(`Cached (DBpedia): ${key.substring(0, 50)}${key.length > 50 ? '...' : ''}`);
  }
  ...
  invalidateWikidataEntry(key: string): boolean {
    ...
    if (deleted) {
      logger.debug(`Invalidated cache entry (Wikidata): ${key.substring(0, 50)}${key.length > 50 ? '...' : ''}`);
    }
    return deleted;
  }
  ...
  invalidateDBpediaEntry(key: string): boolean {
    ...
    if (deleted) {
      logger.debug(`Invalidated cache entry (DBpedia): ${key.substring(0, 50)}${key.length > 50 ? '...' : ''}`);
    }
    return deleted;
  }
